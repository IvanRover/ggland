...
// In-memory fallback (only works for same serverless execution context, which means it will reset constantly, so Firebase is required for Vercel)
...
app.get("/:id", async (req, res, next) => {
  ...
  if (id === "api" || id.length !== 6 || id.startsWith("@") || id.includes(".") || id.startsWith("src")) {
    return next();
  }
  ...
  // Return a generic response or 404
